...
class BudgetPerDayMixin:
  
  def get_budget_calendar(self):
    queryset = self.model.objects.filter(user=self.request.user)


class BaseCalendarMixin:
  """カレンダー関連Mixinの、基底クラス"""
  first_weekday:int = 6  # 0は月曜から、1は火曜から。6なら日曜日からになります。
  week_names:list = ['月', '火', '水', '木', '金', '土', '日']  # これは、月曜日から書くことを想定します。['Mon', 'Tue'...
...
class MonthWithScheduleMixin(MonthCalendarMixin):
  ...
  def get_month_schedules(self, start, end, days):
    ...
    queryset = self.model.objects.filter(**lookup).filter(user=self.request.user)
    print(queryset)

    # {1日のdatetime: 1日のスケジュール全て, 2日のdatetime: 2日の全て...}のような辞書を作る
    day_schedules = {日ごと: [] for 週ごと in days for 日ごと in 週ごと}
  ...
  def get_month_calendar(self):
    ...
    calendar_context['month_day_schedules'] = \
      self.get_month_schedules(month_first, month_last, month_days)
    
    print("calendar_context['month_day_schedules']", calendar_context['month_day_schedules'])
